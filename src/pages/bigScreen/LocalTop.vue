    ...
    created () {
        infraApi.localCity(sessionStorage.getItem('USERID')).then((res) => {
            ...
            if (localData.REGIONNO.slice(2, 6) === '0000') {
                infraApi.dapingAll(localData.REGIONNO.slice(0, 2)).then(that.handleAllSuees.bind(that))
                this.timer = setInterval(function () { infraApi.dapingAll(localData.REGIONNO.slice(0, 2)).then(that.handleAllSuees.bind(that)) }, 5000)
            } else {
                infraApi.dapingAll(localData.REGIONNO).then(that.handleAllSuees.bind(that))
                this.timer = setInterval(function () { infraApi.dapingAll(localData.REGIONNO).then(that.handleAllSuees.bind(that)) }, 5000)
            }
        })
    },
    beforeMount () {
        // let regionBig = window.sessionStorage.getItem('regionTopId')
        // this.areaBig = regionBig
        // let that = this
        // infraApi.dapingAll(that.areaBig).then(that.handleAllSuees.bind(that))
        // this.timer = setInterval(function () { infraApi.dapingAll(that.areaBig).then(that.handleAllSuees.bind(that)) }, 5000)
        // sessionStorage.removeItem('regionTopId')
    },
    methods: {
        handleAllSuees (res) {
